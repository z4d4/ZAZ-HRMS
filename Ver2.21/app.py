...
from blueprints.dm_module.dm import dm_module
import sqlite3
import os

app = Flask(__name__)
app.register_blueprint(dm_module, url_prefix="/dm_module")
...
def home():
    if 'user' in session:
        ...
        return render_template('home.html', user_details=user_details, notifications=notifications)
    else:
...
def get_department_leave_data(department):
    ...
    cursor = con.cursor()
    cursor.execute("""
        SELECT la.Leave_ID, la.User_ID, ei.User_name, la.Start_Date, la.End_Date, la.Status, la.Leave_Reason, la.Reason
    ...
    leave_data = cursor.fetchall()
    con.close()
    print(f"Leave data retrieved for department: {leave_data}")  # Debugging info
    return leave_data
...
def leave_application():
    ...
    if 'Manager' in role:
        ...
        if request.method == 'POST':
            ...
            print(f"Form submitted with action: {action}, Leave ID: {leave_id}, User ID: {user_id}")  # Debugging
            ...
            if action == 'Reject' and reason:
                ...
                flash('Invalid action or missing confirmation.')
...
def update_leave_status(leave_id, status, reason=None):
    ...
    try:
        if reason:
            cursor.execute(
                "UPDATE 'Leave Applications' SET Status = ?, Reason = ? WHERE Leave_ID = ?",
                (status, reason, leave_id)
            )
        else:
            cursor.execute(
                "UPDATE 'Leave Applications' SET Status = ? WHERE Leave_ID = ?",
                (status, leave_id)
            )
        con.commit()
        print(f"Successfully updated Leave ID {leave_id} to status {status}")  # Debugging info
    except Exception as e:
        print(f"Failed to update Leave ID {leave_id}: {e}")
    finally:
        con.close()
...
def apply_for_leave(user_id, start_date, end_date, leave_reason):
    ...
    cursor.execute(
    "INSERT INTO 'Leave Applications' (User_ID, Start_Date, End_Date, Status, Leave_Reason) VALUES (?, ?, ?, 'Submitted', ?)",
    (user_id, start_date, end_date, leave_reason))

    con.commit()
    print(f"Inserted leave application for User ID {user_id} with reason: {leave_reason}")  # Debugging
    con.close()
